    ...
    public class CLobbyManagerScript : MonoBehaviourPunCallbacks
    {
        ...
        public void GetRooms(List<RoomInfo> roomInfo)
        {
            ...
            for (int i = 0; i < roomInfo.Count; i++)
            {
                ...
                RoomElement.GetComponent<CRoomElement>().SetRoomInfo(roomInfo[i].Name, roomInfo[i].PlayerCount, roomInfo[i].MaxPlayers, roomInfo[i].CustomProperties["RoomCreator"].ToString());
            }
        }
